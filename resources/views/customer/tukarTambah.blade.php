...
<div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <form action="{{ route('tukar.tambah.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @if(isset($ponsel))
        <input type="hidden" name="produk_tujuan_id" value="{{ $ponsel->id_ponsel }}">
        @endif
        
        <div class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Informasi Ponsel yang Akan Ditukar</h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Harga Estimasi -->
            <div class="md:col-span-2">
                <label for="harga_estimasi" class="block text-sm font-medium text-gray-700 mb-1">Estimasi Harga Ponsel Anda (Rp) <span class="text-red-500">*</span></label>
                <input type="number" name="harga_estimasi" id="harga_estimasi" value="{{ old('harga_estimasi') }}" min="1000" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-purple-500 focus:border-purple-500" required>
                <p class="text-xs text-gray-500 mt-1">Masukkan estimasi harga ponsel Anda. Tim kami akan meninjau dan memberikan penilaian akhir.</p>
            </div>
        ...
        <div class="mt-8 flex justify-end">
            <a href="{{ url()->previous() }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 mr-3">
                Batal
            </a>
