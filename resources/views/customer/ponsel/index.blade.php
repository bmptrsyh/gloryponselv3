...
@section('content')
   ...
   <div class="container mx-auto mt-8 px-4 flex justify-between space-x-4">
      <form action="" method="GET" class="w-2/4 flex space-x-4">
         <input type="text" name="keyword" placeholder="Cari barang yang anda inginkan..."
   ...
   <section class="container mx-auto px-4 mt-10 mb-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4 items-stretch">
         @forelse ($produk as $produk)
            <a href="{{ route('produk.show', $produk->id_ponsel) }}" class="h-full">
               <div class="bg-white shadow rounded-xl p-3 flex flex-col h-full">
                  <!-- Gambar -->
                  <div class="h-40 flex items-center justify-center mb-4 shadow">
                     <img src="{{ asset($produk->gambar) }}" class="max-h-full  max-w-full object-rounded " alt="{{ $produk->model }}">
                  </div>

                  <!-- Nama Produk -->
                  <p class="font-semibold text-center line-clamp-2 min-h-[48px]">{{ $produk->merk }} {{ $produk->model }}
                  </p>
                  ...
                  <!-- Spacer agar rating selalu di bawah -->
                  <div class="flex-grow"></div>

                  <!-- Rating -->
                  <div class="text-yellow-400 text-sm text-center mt-1">
                     ⭐ {{ number_format($produk->avg ?? 0, 1) }}
                     ({{ $produk->count ?? 0 }} ulasan)
                  </div>
               </div>
            </a>
         @empty
