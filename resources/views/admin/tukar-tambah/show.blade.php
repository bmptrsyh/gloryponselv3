...
<div class="container px-6 mx-auto grid">
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <h4 class="text-lg font-semibold text-gray-700 mb-2">Ponsel Customer</h4>                <img src="{{ asset($pengajuan->gambar) }}" alt="{{ $pengajuan->merk ?? 'N/A' }} {{ $pengajuan->model ?? 'N/A' }}" class="w-full h-auto rounded-lg shadow-md mb-4">
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <div class="flex items-center">
                            @if($pengajuan->produkTujuan)
                                <img src="{{ asset($pengajuan->produkTujuan->gambar) }}" alt="{{ $pengajuan->produkTujuan->merk }} {{ $pengajuan->produkTujuan->model }}" class="w-16 h-16 object-cover rounded-lg mr-4">
                                <div>
                                    <p class="font-medium">{{ $pengajuan->produkTujuan->merk }} {{ $pengajuan->produkTujuan->model }}</p>
                                    <p class="text-sm text-gray-600">Harga: Rp {{ number_format($pengajuan->produkTujuan->harga_jual, 0, ",", ".") }}</p>
                                </div>
                            @else
                ...
                <div class="mt-6">
                    ...
                    <div class="flex items-center">
                        ...
                        <button onclick="openStatusModal({{ $pengajuan->id_tukar_tambah }}, '{{ $pengajuan->status }}')" class="ml-4 px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                            Update Status
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="flex items-center mb-4">
                        <div class="flex-shrink-0 h-10 w-10">
                            <img class="h-10 w-10 rounded-full object-cover" src="{{ $pengajuan->customer->foto_profil_url ?? asset('storage/gambar/customer/default.png') }}" alt="{{ $pengajuan->customer->nama ?? 'Customer' }}">
                        </div>
                        <div class="ml-4">
                            <div class="text-sm font-medium text-gray-900">{{ $pengajuan->customer->nama ?? 'Customer #' . $pengajuan->id_customer }}</div>
            ...
            <div>
                <h3 class="text-xl font-semibold text-gray-700 mb-4">{{ $pengajuan->merk ?? 'N/A' }} {{ $pengajuan->model ?? 'N/A' }}</h3>
                
                <div class="mb-6">
                    <h4 class="text-lg font-semibold text-gray-700 mb-2">Estimasi Harga Ponsel Customer</h4>
                    <p class="text-2xl font-bold text-purple-600">Rp {{ number_format($pengajuan->harga_estimasi ?? 0, 0, ",", ".") }}</p>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-md">
                        <p class="text-gray-700 whitespace-pre-line">{{ $pengajuan->deskripsi ?? 'Tidak ada deskripsi.' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-8 flex justify-between">
            <a href="{{ route('admin.tukar-tambah.index') }}" class="px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition-colors duration-150 bg-white border border-gray-300 rounded-md active:bg-gray-50 hover:bg-gray-50 focus:outline-none focus:shadow-outline-gray">
                Kembali ke Daftar
            </a>
            
            <form action="{{ route('admin.tukar-tambah.destroy', $pengajuan->id_tukar_tambah) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengajuan ini?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-md active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red">
                    Hapus Pengajuan
                </button>
            </form>
        </div>
